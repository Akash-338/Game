...
const gameCommandDispatcher = createAsyncGameCommandDispatcher({
  ...
  sqliteCommands: { snapshot, setReady, setPresence, kickPlayer, updateSettings, reorderPlayers, nextTurn, setDiscussionLocked, revealRoles, chooseRandomWord: (hostToken) => chooseRandomWord(hostToken), getRoomByHostToken, startRound, submitHint, submitVote, submitSkipVote, continueRound, postDirectMessage, endRoom, restartSession, postDiscussionMessage, leaveRoom, uploadPack, setPackEnabled, deletePack, abstainDisconnectedVoter, editHint, removeDiscussionMessage, markMemorableClue, createAlert }
});
...
io.on("connection", (socket) => {
  ...
  socket.on("randomizeWord", async ({ hostToken }, ack) => { try { const result = await gameCommandDispatcher.randomizeWord(hostToken); await emitAuthoritativeRoom(result.roomId); ack?.({ ok: true, word: result.word }); } catch (error) { ackError(ack, error); } });
  ...
  socket.on("restartSession", async ({ hostToken }, ack) => { try { const result = await gameCommandDispatcher.restartSession(hostToken); io.to(`room:${result.roomId}`).emit("sessionRestarted"); for (const candidate of io.sockets.sockets.values()) if (candidate.data.roomId === result.roomId) { candidate.leave(`room:${result.roomId}`); candidate.data = {}; } ack?.({ ok: true }); } catch (error) { ackError(ack, error); } });
